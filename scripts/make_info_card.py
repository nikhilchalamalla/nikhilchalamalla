...
def create_info_card(output_path="info-card.svg"):
    ...
    for i, item in enumerate(lines):
        ...
        if item["type"] == "prompt":
            svg.append(f'  <text class="prompt" x="20" y="{y}">{item["text"]}</text>')
        elif item["type"] == "separator":
            svg.append(f'  <text class="sep" x="20" y="{y}">{item["text"]}</text>')
        elif item["type"] == "header":
            svg.append(f'  <text class="header-name" x="20" y="{y}">{item["val"]}</text>')
        elif item["type"] == "kv":
            is_hl = item["label"] in ["Highlight", "Core Project", "Goal", "Algorithms"]
            val_class = "highlight-val" if is_hl else "val"
            svg.append(f'  <text x="20" y="{y}">')
